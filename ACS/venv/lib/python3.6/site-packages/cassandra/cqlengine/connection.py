...
class Connection(object):
    ...
    def setup(self):
        ...
        try:
            ...
            log.debug(
                format_log_context("connection initialized with internally created session", connection=self.name))
        except NoHostAvailable:
            if self.retry_connect:
                log.warning(
                    format_log_context("connect failed, setting up for re-attempt on first use", connection=self.name))
