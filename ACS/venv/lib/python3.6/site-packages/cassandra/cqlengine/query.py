...
class AbstractQuerySet(object):
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in self.__dict__.items():
            if k in ['_con', '_cur', '_result_cache', '_result_idx', '_result_generator',
                     '_construct_result']:  # don't clone these, which are per-request-execution
        ...
        for arg, val in kwargs.items():
            ...
            if isinstance(operator, InOperator):
                ...
                if not col_op:  # only equal values should be deferred
                    ...
                    clone._deferred_values[
                        column.db_field_name] = val  # map by db field name for substitution in results
...
class ModelQuerySet(AbstractQuerySet):
    ...
    def _validate_select_where(self):
        ...
        if not any(
                w.primary_key or w.has_index for w in equal_ops) and not token_comparison and not self._allow_filtering:
            raise QueryException(
    ...
    def update(self, **values):
        ...
        for name, val in values.items():
            ...
            if col.is_primary_key:
                raise ValidationError(
                    "Cannot apply update to primary key '{0}' for {1}.{2}".format(col_name, self.__module__,
                                                                                  self.model.__name__))
...
class DMLQuery(object):
    ...
    def delete(self):
        ...
        ds = DeleteStatement(self.column_family_name, timestamp=self._timestamp, conditionals=self._conditional,
                             if_exists=self._if_exists)
        for name, col in self.model._primary_keys.items():
