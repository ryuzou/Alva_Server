...
def _create_keyspace(name, durable_writes, strategy_class, strategy_options, connections=None):
    ...
    def __create_keyspace(name, durable_writes, strategy_class, strategy_options, connection=None):
        ...
        if name not in cluster.metadata.keyspaces:
            ...
            log.info(format_log_context("Not creating keyspace %s because it already exists", connection=connection),
                     name)
...
def _sync_table(model, connection=None):
    ...
    if raw_cf_name not in tables:
        log.debug(format_log_context("sync_table creating new table %s", keyspace=ks_name, connection=connection),
                  cf_name)
        ...
        log.debug(format_log_context("sync_table checking existing table %s", keyspace=ks_name, connection=connection),
                  cf_name)
        ...
        for model_name, col in model._columns.items():
            ...
            if col.primary_key or col.primary_key:
                msg = format_log_context("Cannot add primary key '{0}' (with db_field '{1}') to existing table {2}",
        ...
        if db_fields_not_in_model:
            msg = format_log_context("Table {0} has fields not referenced by model: {1}", keyspace=ks_name,
...
def _update_options(model, connection=None):
    ...
    for name, value in model_options.items():
        try:
            ...
            msg = format_log_context("Invalid table option: '%s'; known options: %s", keyspace=ks_name,
                                     connection=connection)
