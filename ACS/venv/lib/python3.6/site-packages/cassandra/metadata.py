...
cql_keywords = set((
    ...
    'functions', 'grant', 'if', 'in', 'index', 'inet', 'infinity', 'initcond', 'input', 'insert', 'int', 'into', 'is',
    'json',
    'key', 'keys', 'keyspace', 'keyspaces', 'language', 'limit', 'list', 'login', 'map', 'materialized', 'modify',
    'nan', 'nologin',
...
class TableMetadata(object):
    ...
    def __init__(self, keyspace_name, name, partition_key=None, clustering_key=None, columns=None, triggers=None,
                 options=None, virtual=False):
...
class SchemaParserV22(_SchemaParser):
    ...
    def get_all_keyspaces(self):
        ...
        for row in self.keyspaces_result:
            ...
            try:
                ...
                log.exception("Error while parsing metadata for keyspace %s. Metadata model will be incomplete.",
                              keyspace_meta.name)
    ...
    def get_table(self, keyspaces, keyspace, table):
        ...
        where_clause = bind_params(" WHERE keyspace_name = %%s AND %s = %%s" % (self._table_name_col,),
                                   (keyspace, table), _encoder)
        ...
        (cf_success, cf_result), (col_success, col_result), (triggers_success, triggers_result) \
            = self.connection.wait_for_responses(cf_query, col_query, triggers_query, timeout=self.timeout,
                                                 fail_on_error=False)
    ...
    def get_function(self, keyspaces, keyspace, function):
        where_clause = bind_params(
            " WHERE keyspace_name = %%s AND function_name = %%s AND %s = %%s" % (self._function_agg_arument_type_col,),
    ...
    def get_aggregate(self, keyspaces, keyspace, aggregate):
        where_clause = bind_params(
            " WHERE keyspace_name = %%s AND aggregate_name = %%s AND %s = %%s" % (self._function_agg_arument_type_col,),
    ...
    def _build_table_metadata(self, row, col_rows=None, trigger_rows=None):
        ...
        try:
            ...
            log.exception("Error while parsing metadata for table %s.%s row(%s) columns(%s)", keyspace_name, cfname,
                          row, col_rows)
    ...
    def _build_index_metadata(column_metadata, row):
        ...
        if index_name or kind:
            ...
            return IndexMetadata(column_metadata.table.keyspace_name, column_metadata.table.name, index_name, kind,
                                 options)
...
class SchemaParserV3(SchemaParserV22):
    ...
    def _build_table_metadata(self, row, col_rows=None, trigger_rows=None, index_rows=None, virtual=False):
        ...
        try:
            ...
            table_meta._exc_info = sys.exc_info()
            log.exception("Error while parsing metadata for table %s.%s row(%s) columns(%s)", keyspace_name, table_name,
                          row, col_rows)
