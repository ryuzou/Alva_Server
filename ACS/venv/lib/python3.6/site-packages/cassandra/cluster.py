    ...
    * EXPERIMENTAL: :class:`cassandra.io.asyncioreactor.AsyncioConnection`
    ...
    def set_max_requests_per_connection(self, host_distance, max_requests):
        ...
        if max_requests < 1 or max_requests > 127 or \
            ...
            raise ValueError(
                "max_requests must be 1-127 and greater than the min_requests for this host_distance (%d)" %
                (self._min_requests_per_connection[host_distance],))
    ...
    def refresh_table_metadata(self, keyspace, table, max_schema_agreement_wait=None):
        ...
        See :meth:`~.Cluster.refresh_schema_metadata` for description of ``max_schema_agreement_wait`` behavior
        """
        if not self.control_connection.refresh_schema(target_type=SchemaTargetType.TABLE, keyspace=keyspace,
                                                      table=table,
                                                      schema_agreement_wait=max_schema_agreement_wait, force=True):
    ...
    def refresh_user_type_metadata(self, keyspace, user_type, max_schema_agreement_wait=None):
        ...
        See :meth:`~.Cluster.refresh_schema_metadata` for description of ``max_schema_agreement_wait`` behavior
        """
        if not self.control_connection.refresh_schema(target_type=SchemaTargetType.TYPE, keyspace=keyspace,
                                                      type=user_type,
                                                      schema_agreement_wait=max_schema_agreement_wait, force=True):
    ...
    def refresh_user_function_metadata(self, keyspace, function, max_schema_agreement_wait=None):
        ...
        See :meth:`~.Cluster.refresh_schema_metadata` for description of ``max_schema_agreement_wait`` behavior
        """
        if not self.control_connection.refresh_schema(target_type=SchemaTargetType.FUNCTION, keyspace=keyspace,
                                                      function=function,
                                                      schema_agreement_wait=max_schema_agreement_wait, force=True):
    ...
    def refresh_user_aggregate_metadata(self, keyspace, aggregate, max_schema_agreement_wait=None):
        ...
        See :meth:`~.Cluster.refresh_schema_metadata` for description of ``max_schema_agreement_wait`` behavior
        """
        if not self.control_connection.refresh_schema(target_type=SchemaTargetType.AGGREGATE, keyspace=keyspace,
                                                      aggregate=aggregate,
                                                      schema_agreement_wait=max_schema_agreement_wait, force=True):
...
class Session(object):
    ...
    def execute(self, query, parameters=None, timeout=_NOT_SET, trace=False, custom_payload=None,
                execution_profile=EXEC_PROFILE_DEFAULT, paging_state=None):
        """
    ...
    def _create_response_future(self, query, parameters, trace, custom_payload, timeout,
        ...
        spec_exec_plan = spec_exec_policy.new_plan(query.keyspace or self.keyspace,
                                                   query) if query.is_idempotent and spec_exec_policy else None
            ...
            with self._lock:
                while new_pool._keyspace != self.keyspace:
                    ...
                    if not set_keyspace_event.is_set() or errors_returned:
                        log.warning("Failed setting keyspace for pool after keyspace changed during connect: %s",
                                    errors_returned)
    ...
    def _refresh_node_list_and_token_map(self, connection, preloaded_results=None,
        ...
        for row in peers_result:
            ...
            if addr in found_hosts:
                log.warning("Found multiple hosts with the same rpc_address (%s). Excluding peer %s", addr,
                            row.get("peer"))
                continue
        ...
        if self._timer is None:
            ...
            if spec_delay >= 0:
                if self._time_remaining is None or self._time_remaining > spec_delay:
                    self._timer = self.session.cluster.connection_class.create_timer(spec_delay,
                                                                                     self._on_speculative_execute)
                    return
