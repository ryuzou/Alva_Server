...
def named_tuple_factory(colnames, rows):
    ...
    try:
        ...
        clean_column_names = list(
            map(_clean_column_name, colnames))  # create list because py3 map object will be consumed by first attempt
...
class Statement(object):
    ...
    def __init__(self, retry_policy=None, consistency_level=None, routing_key=None,
                 ...
                 is_idempotent=False):
        if retry_policy and not hasattr(retry_policy,
                                        'on_read_timeout'):  # just checking one method to detect positional parameter errors
    ...
    def populate(self, max_wait=2.0, wait_for_complete=True, query_cl=None):
        ...
        while True:
            ...
            session_results = self._execute(
                SimpleStatement(self._SELECT_SESSIONS_FORMAT, consistency_level=query_cl), (self.trace_id,), time_spent,
                max_wait)

            # PYTHON-730: There is race condition that the duration mutation is written before started_at the for fast queries
            is_complete = session_results and session_results[0].duration is not None and session_results[
                0].started_at is not None
            ...
            event_results = self._execute(
                SimpleStatement(self._SELECT_EVENTS_FORMAT, consistency_level=query_cl), (self.trace_id,), time_spent,
                max_wait)
